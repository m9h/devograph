...
class TestComputePersistence:
    ...
    def test_diagram_shapes(self):
        hg = _triangle_hypergraph()
        diagrams = devograph.compute_persistence(hg, filtration="clique", max_dim=1)
    ...
    def test_weight_filtration(self):
        hg = _triangle_hypergraph()
        diagrams = devograph.compute_persistence(hg, filtration="weight", max_dim=1)
        assert len(diagrams) == 2

    def test_degree_filtration(self):
        hg = _path_hypergraph()
        diagrams = devograph.compute_persistence(hg, filtration="degree", max_dim=1)
        assert len(diagrams) == 2
...
class TestPersistenceFeatures:
    ...
    def test_landscape_features_shape(self):
        hg = _triangle_hypergraph()
        feats = devograph.persistence_features(
