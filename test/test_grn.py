"""Tests for gene regulatory network data loaders.
...
class TestLoadGRNFromAnndata:
    ...
    def test_basic_load(self, h5ad_path):
        from devograph._grn import load_grn_from_anndata
...
# ---------------------------------------------------------------------------
# 4. load_pando_modules
# ---------------------------------------------------------------------------


class TestLoadPandoModules:
    """Test ``load_pando_modules``."""

    @pytest.fixture
    def coef_csv(self, tmp_path: Path) -> Path:
        """Write a synthetic Pando coefficients CSV."""
        p = tmp_path / "pando_coef.csv"
        with open(p, "w", newline="") as fh:
            writer = csv.writer(fh)
            writer.writerow(["tf", "target", "estimate", "padj"])
            # Significant edges
            writer.writerow(["TF1", "geneA", 0.5, 0.001])
            writer.writerow(["TF1", "geneB", 0.3, 0.01])
            writer.writerow(["TF2", "geneC", 0.8, 0.02])
            # Non-significant edge
            writer.writerow(["TF3", "geneD", 0.1, 0.1])
        return p

    @pytest.fixture
    def modules_csv(self, tmp_path: Path) -> Path:
        """Write a synthetic modules mapping CSV."""
        p = tmp_path / "pando_modules.csv"
        with open(p, "w", newline="") as fh:
            writer = csv.writer(fh)
            writer.writerow(["gene", "module"])
            writer.writerow(["TF1", "mod1"])
            writer.writerow(["geneA", "mod1"])
            writer.writerow(["geneB", "mod1"])
            writer.writerow(["TF2", "mod2"])
            writer.writerow(["geneC", "mod2"])
        return p

    def test_basic_load(self, coef_csv):
        hg = load_pando_modules(coef_csv)
        _is_valid_hypergraph(hg)
        # Only 3 significant edges, 2 TFs (TF1, TF2) -> 2 hyperedges
        assert hg.incidence.shape[1] == 2
        # 5 genes: TF1, TF2, geneA, geneB, geneC (TF3/geneD filtered out)
        assert hg.incidence.shape[0] == 5

    def test_padj_filtering(self, coef_csv):
        """Stricter threshold should exclude more edges."""
        hg = load_pando_modules(coef_csv, padj_threshold=0.005)
        # Only TF1->geneA (padj=0.001) passes
        assert hg.incidence.shape[1] == 1

    def test_with_modules(self, coef_csv, modules_csv):
        hg = load_pando_modules(coef_csv, modules_csv)
        _is_valid_hypergraph(hg)
        assert hg.incidence.shape[1] == 2  # mod1, mod2

    def test_no_significant_edges(self, tmp_path):
        p = tmp_path / "all_ns.csv"
        with open(p, "w", newline="") as fh:
            writer = csv.writer(fh)
            writer.writerow(["tf", "target", "estimate", "padj"])
            writer.writerow(["TF1", "A", 0.5, 0.9])
        with pytest.raises(ValueError, match="No significant edges"):
            load_pando_modules(p)

    def test_missing_column_error(self, tmp_path):
        p = tmp_path / "bad_pando.csv"
        with open(p, "w", newline="") as fh:
            fh.write("tf,target,weight\nTF1,A,0.5\n")
        with pytest.raises(ValueError, match="not found"):
            load_pando_modules(p)

    def test_custom_columns(self, tmp_path):
        p = tmp_path / "custom_pando.csv"
        with open(p, "w", newline="") as fh:
            writer = csv.writer(fh)
            writer.writerow(["regulator", "gene", "beta", "fdr"])
            writer.writerow(["TF1", "A", 0.5, 0.01])
        hg = load_pando_modules(
            p,
            tf_col="regulator",
            target_col="gene",
            estimate_col="beta",
